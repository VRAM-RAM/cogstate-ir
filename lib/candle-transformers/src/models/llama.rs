...
impl Llama {
    ...
    /// Forward pass for training — returns logits for all positions
    /// except the last (no target for it). Shape: [B, S-1, V].
    pub fn forward_train(&self, x: &Tensor, cache: &mut Cache) -> Result<Tensor> {
        let (_b_sz, seq_len) = x.dims2()?;
        let mut x = self.wte.forward(x)?;
        for (block_idx, block) in self.blocks.iter().enumerate() {
            x = block.forward(&x, 0, block_idx, cache)?;
        }
        let x = self.ln_f.forward(&x)?;
        let x = x.narrow(1, 0, seq_len.saturating_sub(1))?.contiguous()?;
        let logits = self.lm_head.forward(&x)?;
        logits.to_dtype(DType::F32)
    }
